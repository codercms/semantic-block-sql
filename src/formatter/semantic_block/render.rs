...
use crate::formatter::tokens::{
    TokenRole, next_non_comment, normalize_comment_trailing_whitespace, previous_non_comment,
};
...
pub(in crate::formatter) fn render_token(
    ...
    if token.is_comment() {
        return normalize_comment_trailing_whitespace(token.text).into_owned();
    }
