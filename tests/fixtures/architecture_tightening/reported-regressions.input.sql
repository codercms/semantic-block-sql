...
BEGIN ATOMIC
    ...
    combined AS (
        ...
        FROM (SELECT 'by_watch_rel'::user_watch_notification_reason AS src, * FROM user_new_related_items_by_watch
            ...
            SELECT 'by_list'::user_watch_notification_reason AS src, * FROM user_new_items_in_planning) s
    )
    SELECT
        c_uid,
        id_seq,
...
BEGIN ATOMIC
    ...
    combined AS (
        ...
        FROM (SELECT 'by_watch'::user_watch_notification_reason AS src, * FROM user_new_episodes_by_watch

            UNION ALL

            SELECT 'by_list'::user_watch_notification_reason AS src, * FROM user_new_episodes_by_list) s
